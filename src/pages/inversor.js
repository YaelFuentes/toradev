...
const MovingImg = ({ title, img, link }) => {
  ...
  return (
    ...
    >
      ...
      <FramerImage
      style={{x:x , y:y}}
      src={img} alt={title} className='w-96 h-auto hidden absolute rounded-lg' />
    </Link>
  )
}
...
const Article = ({ img, title, date, link }) => {
  return (
    <li className='relative w-full p-4 py-6 my-4 rounded-xl flex items-center justify-between bg-light text-dark first:mt-0 border border-solid border-dark border-r-4 border-b-4'>
      ...
      <span className='text-primay font-semibold pl-4'>{date}</span>
    </li>
  )
}
...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
  return (
    <li className='col-span-1 w-full p-4 bg-light border border-solid border-dark rounded-2x1'>
      ...
      <Link href={link} target="_blank">
        <h2 className='capitalize text-2x1 font-bold my-2 hover:underline'>{title}</h2>
      </Link>
...
const inversor = () => {
  return (
    <>
      ...
      <main className='w-full mb-16 flex flex-col items-center justify-center oweflow-hidden'>
